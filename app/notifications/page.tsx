...
const TYPE_ICON: Record<NotificationType, { emoji: string; bg: string; color: string }> = {
    like:         { emoji: '👍', bg: 'bg-blue-100',   color: 'text-blue-600'   },
    comment:      { emoji: '💬', bg: 'bg-green-100',  color: 'text-green-600'  },
    endorsement:  { emoji: '⭐', bg: 'bg-amber-100',  color: 'text-amber-600'  },
    connection:   { emoji: '🤝', bg: 'bg-blue-100',   color: 'text-blue-600'   },
    profile_view: { emoji: '👁️', bg: 'bg-gray-100',   color: 'text-gray-600'   },
    job_alert:    { emoji: '💼', bg: 'bg-indigo-100', color: 'text-indigo-600' },
    repost:       { emoji: '🔁', bg: 'bg-green-100',  color: 'text-green-600'  },
    mention:      { emoji: '🗣️', bg: 'bg-purple-100', color: 'text-purple-600' },
    anniversary:  { emoji: '🎉', bg: 'bg-amber-100',  color: 'text-amber-600'  },
};
...
function NotificationRow({ n, onRead }: { n: Notification; onRead: (id: number) => void }) {
    ...
    return (
        <Link
            href={n.link ?? '#'}
            onClick={() => onRead(n.id)}
            className={`flex gap-3 p-4 rounded-lg hover:bg-gray-50 transition-colors group ${!n.read ? 'bg-blue-50/40' : ''}`}
        >
            {/* Avatar or icon */}
            ...
            <div className="flex-1 min-w-0">
                <p className={`text-sm leading-snug ${!n.read ? 'font-semibold text-gray-900' : 'text-gray-800'}`}>
                    {n.message}
                </p>
                {n.subtext && (
                    <p className="text-xs text-gray-500 mt-0.5 italic leading-snug line-clamp-2">{n.subtext}</p>
                )}
                <p className={`text-xs mt-1 ${!n.read ? 'text-[#0a66c2] font-semibold' : 'text-gray-400'}`}>
                    {timeAgo(n.timestamp)}
                </p>
            </div>
            ...
            <div className="max-w-3xl mx-auto px-4 py-5">
                ...
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        {unreadCount > 0 && (
                            <p className="text-xs text-gray-500 mt-0.5">{unreadCount} new notification{unreadCount !== 1 ? 's' : ''}</p>
                        )}
                    </div>
                    {unreadCount > 0 && (
                        <button
                ...
                {/* Filter tabs */}
                <div className="bg-white rounded-lg border border-[#e0dfdc] flex mb-4 overflow-x-auto">
                    {TABS.map(({ key, label }) => (
                        <button
                            key={key}
                ...
                {/* Notification list */}
                <div className="bg-white rounded-lg border border-[#e0dfdc] divide-y divide-gray-100 overflow-hidden">
                    {filtered.length === 0 ? (
                        <div className="p-8 text-center">
                            <p className="text-sm font-semibold text-gray-700">Nothing here.</p>
                            <p className="text-xs text-gray-400 mt-1">The savanna is quiet. For now.</p>
                        </div>
                    ) : (
                        <>
                            {unread.length > 0 && (
